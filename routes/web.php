...
// setting page show
Route::get('/addSettingspage', function(){
    return view('backend.settings.addSettings');
})->name('addSettingspage');

// addSettingspage
Route::post('/addSettings',[SettingController::class, 'addSettings'])->name('addSettings');
...
// addCurt
Route::get('/addCurt', function () {

    // $arrivals = Arrival::all();
    // $amazon = [];
    // foreach ($settings as $setting) {
    //     $country = $setting->associate_country;
    //     $id = $setting->associate_id;
    //     $amazon = "https://www.amazon.".$country."/gp/aws/cart/add.html?AssociateTag=".$id."&ASIN.1=".$arrivals[0]['asin']."&Quantity.1=1";
    //     array_push([0],$amazon);
    // }
    // https://www.amazon.com/gp/aws/cart/add.html?AssociateTag=0205-21&ASIN.1=B082PN4X5J&Quantity.1=1
    $settings = Setting::all();
    return view('backend.settings.settings', compact('settings'));
    // compact('arrivals')
})->name('addCurt');
